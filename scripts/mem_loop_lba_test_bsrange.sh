...
#LBA测试参数保留一份到dmesg日志
echo "${LBA_TOOLS} -c -D -K -w on -B ${bsrange} -S ${cluster_sectors} -V once -T 10 -L ${bwlimit} ${MEM_LOOP}"
echo "${LBA_TOOLS} -c -D -K -w on -B ${bsrange} -S ${cluster_sectors} -V once -T 10 -L ${bwlimit} ${MEM_LOOP}" > /dev/kmsg
...
do
	...
	#内存LBA测试
	${LBA_TOOLS} -c -D -K -w on -B ${bsrange} -S ${cluster_sectors} -V once -T 10 -L ${bwlimit} ${MEM_LOOP}
