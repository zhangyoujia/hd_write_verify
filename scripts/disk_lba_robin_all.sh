...
#LBA测试参数保留一份到dmesg日志
echo "${LBA_TOOLS} -c -D -K -w on -U on -t on -B ${bsrange} -S ${cluster_sectors} -V all -T 10 -L ${bwlimit} -P robin ${stripe_disk_list}"
echo "${LBA_TOOLS} -c -D -K -w on -U on -t on -B ${bsrange} -S ${cluster_sectors} -V all -T 10 -L ${bwlimit} -P robin ${stripe_disk_list}" > /dev/kmsg

#块存储稳定性测试和数据一致性校验
${LBA_TOOLS} -c -D -K -w on -U on -t on -B ${bsrange} -S ${cluster_sectors} -V all -T 10 -L ${bwlimit} -P robin ${stripe_disk_list}
