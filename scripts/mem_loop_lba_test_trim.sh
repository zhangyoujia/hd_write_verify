...
#LBA测试参数保留一份到dmesg日志
echo "${LBA_TOOLS} -c -D -K -R 33 -w on -t on -n 3 -S ${cluster_sectors} -V all -T 10 -L ${bwlimit} ${MEM_LOOP}"
echo "${LBA_TOOLS} -c -D -K -R 33 -w on -t on -n 3 -S ${cluster_sectors} -V all -T 10 -L ${bwlimit} ${MEM_LOOP}" > /dev/kmsg
...
do
	...
	#内存LBA测试
	${LBA_TOOLS} -c -D -K -R 33 -w on -t on -n 3 -S ${cluster_sectors} -V all -T 10 -L ${bwlimit} ${MEM_LOOP}
