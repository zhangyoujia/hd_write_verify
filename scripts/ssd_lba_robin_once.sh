...
#LBA测试参数保留一份到dmesg日志
echo "${LBA_TOOLS} -c -D -K -s on -U on -W on -B ${bsrange} -S ${cluster_sectors} -V once -T 64 -L ${bwlimit} -P robin ${stripe_disk_list}"
echo "${LBA_TOOLS} -c -D -K -s on -U on -W on -B ${bsrange} -S ${cluster_sectors} -V once -T 64 -L ${bwlimit} -P robin ${stripe_disk_list}" > /dev/kmsg

#块存储稳定性测试和数据一致性校验
${LBA_TOOLS} -c -D -K -s on -U on -W on -B ${bsrange} -S ${cluster_sectors} -V once -T 64 -L ${bwlimit} -P robin ${stripe_disk_list}
