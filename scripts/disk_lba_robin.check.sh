...
#LBA测试参数保留一份到dmesg日志
echo "${LBA_TOOLS} -c -D -K -V check -T 1 -L ${bwlimit} -P robin ${stripe_disk_list}"
echo "${LBA_TOOLS} -c -D -K -V check -T 1 -L ${bwlimit} -P robin ${stripe_disk_list}" > /dev/kmsg

#块存储数据一致性校验
${LBA_TOOLS} -c -D -K -V check -T 1 -L ${bwlimit} -P robin ${stripe_disk_list}
