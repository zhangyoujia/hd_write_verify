...
#LBA测试参数保留一份到dmesg日志
echo "${LBA_TOOLS} -c -D -K -R 33 -w on -t on -n 3 -S ${cluster_sectors} -V all -T 10 -L ${bwlimit} ${LBA_FILE}"
echo "${LBA_TOOLS} -c -D -K -R 33 -w on -t on -n 3 -S ${cluster_sectors} -V all -T 10 -L ${bwlimit} ${LBA_FILE}" > /dev/kmsg
...
do
	...
	#文件存储稳定性测试和数据一致性校验
	${LBA_TOOLS} -c -D -K -R 33 -w on -t on -n 3 -S ${cluster_sectors} -V all -T 10 -L ${bwlimit} ${LBA_FILE}
